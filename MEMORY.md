...
- **时区时间差规则（2026-07-03 确立）**：新闻发布时间 ≠ 事件发生时间
  - 美股收盘：美东 16:00 = 北京时间次日 04:00/05:00
...
- **DATE LOCK（2026-07-14 确立）**：cron 任务只接受过去 3 天内（今天+2天前）发布的新闻
  - 根因：2026-07-14 下午任务抓到了 05-05 的新闻，时间锁失效
  - 修复：4 个 cron 任务 prompt 加入 `DATE LOCK` + `SCOPE: ... from the LAST 3 DAYS`
  - 文档同步更新：`docs/news-intel-task-guide.md` 时间过滤章节
  - 超过 3 天的搜索结果直接丢弃，不写入 JSON
