...
**时间过滤（硬性要求，DATE LOCK）：**
- **时间窗口：只接受过去 3 天内（今天 + 2 天前）发布的新闻**
- **搜索到的新闻如果超过 3 天，直接丢弃，不写入 JSON**
- 每轮搜索必须附加时间过滤
- 支持 `after:` 的引擎：关键词后加 `after:YYYY-MM-DD`（日期 = 今天-2天）
- 不支持 `after:` 的引擎：用「过去72小时」「最近3天」等筛选
- **无时间过滤或结果超过3天的搜索结果视为无效，直接丢弃**
...
| 字段 | 要求 | 说明 |
|------|------|------|
| `id` | 必填 | `YYYY-MM-DD-英文短描述` |
| `date` | 必填 | 事件实际发生日期（非新闻发布日期） |
| `title` | 必填 | 短标题 |
| `source` | 必填 | 具体媒体名称，不能模糊 |
| **`url`** | **必填** | **原始文章链接，必须提取并填写** |
| `type` | 必填 | 枚举值：市场消息/产品数据/行业分析/重大事件/公司公告/财报业绩/产业链跟踪/政策事件 |
| `impact` | 必填 | 枚举值：bullish/bearish/neutral |
| `importance` | 必填 | S/A/B/C |
| `product` | 必填 | 产品/公司/主题 |
| `summary` | 必填 | 事实摘要 |
...
- 搜索到新闻后，**必须提取并填写原始文章链接**
- 只允许 `http://` 或 `https://`
- **禁止伪造 URL**：没有链接时不要编造不存在的网址
- **快讯再搜索规则（2026-09-17 确立）**：若合格新闻（尤其是 CFM闪存市场 等聚合页上的快讯）没有独立文章 URL，或其唯一 URL 是库中已占用的列表页/feed 页（会触发 `assertUniqueIntelUrls` 硬冲突），**不得直接丢弃**——用该快讯的标题/关键词追加一轮再搜索（可英+中各试一次），找其他媒体的独立报道链接；确认该 URL 未被库占用后，以该媒体为 `source` 写入（白名单媒体优先）。再搜索后仍无独立 URL，才丢弃该条。
- 如果确实找不到可访问的原文链接（如闭门会议纪要、私人研报），将 `url` 设为空字符串 `""`，并在 `source` 字段详细注明来源
- **缺少 URL 的记录会降低情报库可信度，必须在候选阶段就补充完整**

**教训（2026-09-17）**：早班任务曾因 CFM 快讯页共享 URL 冲突，一次性丢弃 6 条合格快讯（英特尔CEO内存警告、美光512GB RDIMM、三星HBM双轨制、SK海力士工资协议、铠侠上市声明等），后经快讯标题再搜索全部找回独立来源（澎湃、财联社、美光IR官方稿、新浪/彭博等）补录入库。聚合页快讯 ≠ 无源新闻。

### 已有记录 URL 缺失问题

**现状**：情报库中部分历史记录缺少 `url` 字段，主要集中在 2026-07 至 2026-08 期间。修复方案：
1. 新增记录严格执行 URL 必填
2. 历史缺失 URL 的记录通过批量搜索补充（一次性修复任务）
3. 无法找到原文链接的历史记录，保留 `source` 但 `url` 留空
