...
`url` 为**必填字段**。每条情报记录必须包含原文链接。

- 搜索到新闻后，**必须提取并填写原始文章链接**
- 只允许 `http://` 或 `https://`
- 不要填网盘、本地文件路径、聊天记录截图路径
- **禁止伪造 URL**：没有链接时不要编造不存在的网址
- 如果找不到可核对的原文链接，将完整记录放入 `data/archive/intel/` 待核实，不进入 `content/intel/` 发布库。
- **无 URL 的新闻禁止写入 JSON**（脚本会硬拒绝）
- 必须打开原文核对标题、发布日期和摘要中的关键事实；历史文件中存在过的网址也必须复核，HTTP 200 不代表内容匹配。
- 首页、栏目页、标签页、行情页不能代替新闻原文。
- 相同链接（包括 http/https、www、跟踪参数变体）须人工复核。同一文章重复摘录应合并；错配链接应更正，不能把 URL 清空后继续发布。
- 同一篇早报包含多个已核实事件时，整理成一条早报情报，摘要列出相关事实。不同文章报道同一事件须结合新增信息判断，不能仅凭字符重叠率自动删除。
- `npm run validate:data` 会拒绝缺失/非文章链接、重复 URL、重复记录及非法枚举值，并输出定位信息。
- `node scripts/clean-duplicates.mjs` 会备份输入后清理已确认重复记录；未确认的重复 URL 会阻止写入，须先核对。
- `node scripts/dedup-and-merge.mjs` 保留候选文件供追溯，重复运行不会重复收录，不批量删除文件。
